...
function MiniBar({ value, max }: { value: number; max: number }) {
  ...
  return (
    <div className="flex-1 flex flex-col justify-end" style={{ height: 56 }}>
      <div
        className="w-full rounded-t-sm transition-all duration-300 bg-brand"
...
function WeekChangeBadge({ change }: { change: number | null }) {
  if (change === null) return <span className="text-ink-soft text-xs">no prior data</span>
  if (change === 0) return (
...
export default function VisitStatsCard({ slug, restaurantName }: { slug: string; restaurantName: string }) {
  ...
  if (error || !stats) {
    return (
      <div className="mt-4 p-4 rounded-xl bg-sunken border border-line/8 text-ink-soft text-xs">
  ...
  return (
    <div className="mt-4 space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-ink text-xs font-semibold uppercase tracking-wide flex items-center gap-1.5">
          <BarChart3 className="w-3.5 h-3.5 text-brand-ink" /> Page Visit Analytics
        </p>
      ...
      {/* Visit stat cards */}
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-sunken border border-line/5 rounded-xl p-3">
          ...
          <p className="text-ink text-xl font-bold">{stats.thisWeek.toLocaleString()}</p>
          <p className="text-ink-soft text-[11px] mt-0.5 leading-tight">Visits this week</p>
        </div>
        <div className="bg-sunken border border-line/5 rounded-xl p-3">
          <div className="w-7 h-7 rounded-lg bg-sky-400/10 flex items-center justify-center mb-2">
            <Eye className="w-3.5 h-3.5 text-sky-400" />
          </div>
          <p className="text-ink text-xl font-bold">{stats.totalThisMonth.toLocaleString()}</p>
          <p className="text-ink-soft text-[11px] mt-0.5 leading-tight">Visits this month</p>
        </div>
        <div className="bg-sunken border border-line/5 rounded-xl p-3">
          <div className="w-7 h-7 rounded-lg bg-emerald-400/10 flex items-center justify-center mb-2">
            <TrendingUp className="w-3.5 h-3.5 text-emerald-400" />
          </div>
          <p className="text-ink text-xl font-bold">{stats.allTime.toLocaleString()}</p>
          <p className="text-ink-soft text-[11px] mt-0.5 leading-tight">All-time visits</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-sunken border border-line/5 rounded-xl p-3">
          ...
          <p className="text-ink text-xl font-bold">{stats.clicksThisWeek.toLocaleString()}</p>
          <p className="text-ink-soft text-[11px] mt-0.5 leading-tight">Clicks this week</p>
        </div>
        <div className="bg-sunken border border-line/5 rounded-xl p-3">
          <div className="w-7 h-7 rounded-lg bg-violet-400/10 flex items-center justify-center mb-2">
            <MousePointerClick className="w-3.5 h-3.5 text-violet-500" />
          </div>
          <p className="text-ink text-xl font-bold">{stats.clicksThisMonth.toLocaleString()}</p>
          <p className="text-ink-soft text-[11px] mt-0.5 leading-tight">Clicks this month</p>
        </div>
        <div className="bg-sunken border border-line/5 rounded-xl p-3">
          <div className="w-7 h-7 rounded-lg bg-violet-400/10 flex items-center justify-center mb-2">
            <MousePointerClick className="w-3.5 h-3.5 text-violet-500" />
          </div>
          <p className="text-ink text-xl font-bold">{stats.allTimeClicks.toLocaleString()}</p>
          <p className="text-ink-soft text-[11px] mt-0.5 leading-tight">All-time clicks</p>
        </div>
      </div>

      {/* 7-day bar chart */}
      <div className="bg-sunken border border-line/5 rounded-xl p-4">
        <p className="text-ink text-xs font-medium mb-4">Daily Visits — Last 7 Days</p>
        ...
        <div className="flex justify-between mt-2">
          <span className="text-ink-soft/50 text-[10px]">{last7[0]?.date.slice(5)}</span>
          <span className="text-ink-soft/50 text-[10px]">{last7[last7.length - 1]?.date.slice(5)}</span>
        </div>
        {stats.thisWeek === 0 && (
          <p className="text-center text-ink-soft text-xs mt-3 pt-3 border-t border-line/5">
