...
export default async function ClaimedShowcase() {
  ...
  return (
    <section className="bg-sunken py-16 sm:py-20">
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        <div className="text-center mb-12">
          <p className="text-brand-ink text-xs font-semibold uppercase tracking-widest mb-2">Verified &amp; Claimed</p>
          ...
          <p className="text-ink-soft text-sm max-w-xl mx-auto mt-4">
        ...
        <div className="bg-surface rounded-3xl overflow-hidden border border-line/5 shadow-warm-lg">
          {restaurants.map((r, i) => {
            ...
            const photo = (
              <div className="relative w-full h-64 sm:h-full min-h-[280px] bg-page">
            ...
            const details = (
              <div className="flex flex-col justify-center p-8 sm:p-10">
                <div className="flex items-center gap-2 mb-1">
                  <h3 className="font-serif text-2xl sm:text-3xl font-bold text-ink">{r.name}</h3>
                  <span title="Verified owner" className="shrink-0">
                    <BadgeCheck className="w-5 h-5 text-[#2563eb]" />
                  </span>
                </div>
                <p className="flex items-center gap-1.5 text-ink-soft text-xs font-semibold uppercase tracking-widest mb-4">
                  <MapPin className="w-3.5 h-3.5 text-brand-ink" />
                  {r.city}, {r.stateCode}
                </p>

                {(r.rating != null || priceLabel) && (
                  <div className="flex items-center gap-3 mb-4 text-sm text-ink">
                    {r.rating != null && (
                      <span className="flex items-center gap-1 font-semibold">
                        <Star className="w-4 h-4 text-amber-400 fill-amber-400" />
                        {r.rating.toFixed(1)}
                        <span className="text-ink-soft font-normal">({r.reviewCount.toLocaleString()})</span>
                      </span>
                    )}
                    {priceLabel && <span className="text-ink-soft">{priceLabel}</span>}
                  </div>
                )}
                ...
                {activeAmenities.length > 0 && (
                  <div className="flex flex-wrap items-center gap-3 mb-5">
                    {activeAmenities.slice(0, 6).map(({ key, label, Icon }) => (
                      <span key={key} title={label} className="w-9 h-9 rounded-full bg-brand/10 flex items-center justify-center">
                ...
                {r.description && (
                  <p className="text-ink-soft text-sm leading-relaxed mb-6">{r.description}</p>
                )}

                <Link
                  href={href}
                  className="inline-flex items-center gap-1.5 self-start px-5 py-2.5 rounded-none bg-contrast hover:bg-contrast-hi text-white text-sm font-semibold transition-colors"
                >
