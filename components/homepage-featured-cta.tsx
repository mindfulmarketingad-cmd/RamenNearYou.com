      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-[#B0B3BB] text-sm mt-2 max-w-2xl">
        ...
        <div className="relative bg-gradient-to-br from-[#1E2026] via-[#1E2026] to-[#241a26] rounded-3xl border border-amber-500/30 overflow-hidden shadow-2xl shadow-amber-900/20 ring-1 ring-amber-500/20">
          ...
          <div className="relative">
            <div className="flex overflow-x-auto snap-x snap-mandatory scrollbar-hide">
              {Array.from({ length: PHOTO_COUNT }).map((_, i) => (
                <div
                  key={i}
                  className="relative shrink-0 w-[85%] sm:w-[55%] md:w-[42%] lg:w-[32%] xl:w-[26%] h-64 sm:h-72 snap-start bg-gradient-to-br from-[#2a1f2e] via-[#1E2026] to-[#1a2420] border-r border-white/5 last:border-r-0 overflow-hidden"
                >
                  {/* Centered icon */}
                  ...
                  {/* Photo number badge */}
                  <span className="absolute bottom-3 right-3 px-2 py-0.5 rounded-md bg-black/60 text-white/60 text-xs font-medium backdrop-blur-sm">
                    {i + 1} / {PHOTO_COUNT}
                  </span>
                </div>
              ))}
            ...
            <div className="absolute top-4 left-4 flex items-center gap-1.5 px-3 py-1 rounded-full bg-gradient-to-r from-amber-400 to-amber-500 text-[#1E2026] text-xs font-bold shadow-xl shadow-amber-500/40 z-10">
              <Crown className="w-3.5 h-3.5" />
              Homepage Featured
            </div>

            {/* Scroll hint indicator */}
            <div className="absolute top-4 right-4 flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-black/40 backdrop-blur-sm text-white/60 text-xs font-medium z-10">
            ...
            <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6">
              <div className="flex-1 min-w-0">
                <div className="flex flex-wrap items-center gap-2 mb-2">
                  <h3 className="font-serif text-2xl sm:text-3xl font-bold text-white/70 italic">
                ...
                <p className="text-[#B0B3BB] text-base leading-relaxed mb-4 max-w-2xl">
                ...
                <div className="flex flex-wrap gap-2">
                  {['Nationwide Top Placement', '8 Photo Gallery', 'Verified Badge', 'Priority Support', 'Search Boost', 'Featured Email Inclusion'].map(tag => (
                    <span key={tag} className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-white/5 border border-white/10 text-[#B0B3BB]">
                      {tag}
                    </span>
                  ))}
                </div>
              ...
              {/* CTA panel */}
              <div className="shrink-0 lg:w-64 flex flex-col gap-3 lg:border-l lg:border-white/5 lg:pl-6">
                ...
                <Link
                  href="/featured/apply"
                  className="inline-flex items-center justify-center gap-2 px-5 py-3.5 rounded-xl bg-gradient-to-r from-amber-400 to-amber-500 hover:from-amber-300 hover:to-amber-400 text-[#1E2026] text-sm font-bold transition-all shadow-lg shadow-amber-500/30"
                >
                  <Crown className="w-4 h-4" />
                  Get Featured
                </Link>
                <p className="text-white/30 text-xs text-center">No commitment · Cancel anytime</p>
              </div>
            </div>
          </div>
        </div>
