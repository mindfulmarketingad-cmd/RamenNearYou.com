    ...
    <>
      <button
        ...
        className={`inline-flex items-center gap-2 px-4 py-2 rounded-full border text-sm font-semibold transition-colors disabled:opacity-60 ${
          saved
            ? 'bg-brand/10 border-brand/40 text-brand-ink'
            : 'bg-surface border-line/12 text-ink hover:border-brand/50'
        }`}
      >
        <Heart className={`w-4 h-4 transition-all ${saved ? 'fill-brand text-brand-ink' : ''}`} />
