...
export default function RecentlyAdded() {
  ...
  return (
    <section className="py-20 px-4 sm:px-6 lg:px-8 bg-surface noise-texture" ref={sectionRef}>
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-12 fade-up">
          <p className="text-brand-ink text-xs font-medium uppercase tracking-widest mb-3">Fresh Listings</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {restaurants.map((r) => (
            <div
              key={r.id}
              className="fade-up group flex flex-col rounded-xl bg-sunken border border-line/5 overflow-hidden hover:border-brand/50 transition-all duration-200 hover:-translate-y-1 hover:shadow-xl hover:shadow-black/30"
            >
              {/* Photo */}
              <div className="relative h-48 overflow-hidden">
                ...
                />
                <div className="absolute inset-0 bg-gradient-to-t from-sunken to-transparent" />
              </div>

              {/* Content */}
              <div className="flex flex-col flex-1 p-5 gap-3">
                <div className="flex items-start justify-between gap-2">
                  <h3 className="font-semibold text-ink text-base leading-snug">{r.name}</h3>
                ...
                <div className="mt-auto pt-2">
                  <button className="w-full py-2.5 rounded-none border border-brand/50 text-brand-ink text-sm font-medium hover:bg-brand hover:text-ink transition-all duration-200">
                    View Restaurant
                  </button>
                </div>
              </div>
