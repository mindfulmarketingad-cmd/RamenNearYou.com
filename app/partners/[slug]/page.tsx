          ...
          <div className="max-w-3xl mx-auto">
            <div>
              ...
              <nav aria-label="Breadcrumb" className="flex flex-wrap items-center gap-1.5 text-xs text-ink-soft mb-5">
                ...
                <Link href="/partners" className="hover:text-brand-ink transition-colors">Partners</Link>
                <ChevronRight className="w-3 h-3" />
                <span className="text-ink">{p.name}</span>
              </nav>

              {/* Hero */}
              <div className="bg-surface rounded-2xl border border-line/5 overflow-hidden shadow-warm mb-6">
                <div className="relative w-full h-56 sm:h-72 bg-page">
                ...
                <div className="p-6 sm:p-8">
                  <div className="flex items-start gap-2 mb-1 flex-wrap">
                    <h1 className="font-serif text-2xl sm:text-3xl font-bold text-ink">{p.name}</h1>
                    {p.verified && (
                  ...
                  <p className="flex items-center gap-1.5 text-ink-soft text-sm mb-3">
                    <MapPin className="w-4 h-4 text-brand-ink shrink-0" />
                    <a href={directionsUrl} target="_blank" rel="noopener noreferrer" className="hover:text-brand-ink hover:underline">
                  ...
                  <div className="flex items-center gap-3 flex-wrap text-sm mb-5">
                    {p.rating != null && (
                      <span className="flex items-center gap-1 font-semibold text-ink">
                        ...
                        {p.rating.toFixed(1)}
                        <span className="text-ink-soft font-normal">({p.reviewCount.toLocaleString()} reviews)</span>
                      </span>
                    )}
                    {openStatus && (
                      <span
                    ...
                    {p.type && <span className="text-ink-soft">{p.type}</span>}
                  </div>
                    ...
                    {actions.map(({ href, label, Icon, primary }) => (
                      <a
                        ...
                        className={`inline-flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-semibold transition-colors ${
                          primary
                            ? 'bg-contrast hover:bg-contrast-hi text-white'
                            : 'bg-sunken hover:bg-page text-ink border border-line/8'
                        }`}
                      >
              ...
              <div className="grid sm:grid-cols-2 gap-4 mb-6">
                {p.hours && (
                  <div className="bg-surface rounded-2xl border border-line/5 p-6">
                    <h2 className="font-serif text-lg font-bold text-ink mb-3">Hours</h2>
                    <table className="w-full text-sm">
                      <tbody>
                        {DAYS.map(day => {
                          ...
                          return (
                            <tr key={day} className="border-b border-line/5 last:border-0">
                              <th scope="row" className="text-left py-1.5 font-medium text-ink w-28">{day}</th>
                              <td className={`py-1.5 text-right ${closed ? 'text-ink-soft' : 'text-ink'}`}>
                    ...
                    {p.typicalTimeSpent && (
                      <p className="text-xs text-ink-soft mt-3">{p.typicalTimeSpent}</p>
                    )}
                  </div>
                )}
                ...
                {rps && rpsTotal > 0 && (
                  <div className="bg-surface rounded-2xl border border-line/5 p-6">
                    <h2 className="font-serif text-lg font-bold text-ink mb-3">Rating Breakdown</h2>
                      ...
                      {['5', '4', '3', '2', '1'].map(star => {
                        ...
                        return (
                          <div key={star} className="flex items-center gap-2 text-xs">
                            <span className="w-6 text-ink-soft tabular-nums">{star}★</span>
                            <div className="flex-1 h-2 rounded-full bg-page overflow-hidden">
                              <div className="h-full rounded-full bg-brand" style={{ width: `${w}%` }} />
                            </div>
                            <span className="w-16 text-right text-ink-soft tabular-nums">{n.toLocaleString()}</span>
                          </div>
                        )
                      })}
                    </div>
              ...
              {/* Map */}
              <div className="bg-surface rounded-2xl border border-line/5 overflow-hidden mb-6">
                <div className="h-64">
              ...
              {amenityGroups.length > 0 && (
                <div className="bg-surface rounded-2xl border border-line/5 p-6 sm:p-8 mb-6">
                  <h2 className="font-serif text-xl font-bold text-ink mb-4">Features &amp; Amenities</h2>
                  <div className="grid sm:grid-cols-2 gap-x-8 gap-y-5">
                    {amenityGroups.map(g => (
                      <div key={g.group}>
                        <p className="text-[10px] font-bold uppercase tracking-widest text-brand-ink mb-2">{g.group}</p>
                        <ul className="space-y-1">
                          {g.items.map(i => (
                            <li key={i.key} className="flex items-start gap-2 text-sm text-ink">
                ...
                <div className="bg-surface rounded-2xl border border-line/5 p-6 sm:p-8 mb-6">
                  <div className="rounded-xl border border-brand/30 bg-gradient-to-br from-brand/8 to-brand/14 p-5">
                    <div className="flex items-center gap-2 mb-1.5">
                      <BadgeCheck className="w-4 h-4 text-brand-ink" />
              ...
              {/* Long-form editorial */}
              <article className="bg-surface rounded-2xl border border-line/5 p-6 sm:p-8 mb-6">
                {sections.map((s, i) => (
                  <section key={s.id} id={s.id} className={i > 0 ? 'mt-8' : ''}>
                    ...
                    {s.paragraphs.map((html, j) => (
                      <p
                        key={j}
                        className="text-ink-mid text-[15px] leading-relaxed mb-3.5"
              ...
              {isPho && nearby.length > 0 && (
                <div className="bg-surface rounded-2xl border border-line/5 p-6 sm:p-8 mb-6">
                  <h2 className="font-serif text-xl font-bold text-ink mb-1">More Pho Near {p.city}</h2>
                  ...
                  <div className="space-y-3">
                    {nearby.map(o => (
                      <Link
                        ...
                        className="flex items-start gap-3 p-3 rounded-xl bg-raised border border-line/8 hover:border-brand/40 transition-colors group"
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="font-semibold text-sm text-ink group-hover:text-brand-ink transition-colors truncate">{o.name}</p>
                          <p className="text-xs text-ink-soft mt-0.5 truncate">{o.city}, {o.stateCode} · {o.distanceMiles.toFixed(1)} mi</p>
                          {o.rating != null && (
                            <span className="flex items-center gap-0.5 text-xs text-ink-soft mt-1">
                              <Star className="w-3 h-3 fill-brand text-brand-ink" />
                              {o.rating.toFixed(1)}
              ...
              {/* Cross-links */}
              <div className="bg-surface rounded-2xl border border-line/5 p-6 sm:p-8">
                <h2 className="font-serif text-xl font-bold text-ink mb-3">Keep Exploring</h2>
                <div className="flex flex-wrap gap-x-5 gap-y-2 text-sm">
                  <Link href="/partners" className="text-brand-ink hover:underline">All partner restaurants</Link>
                  {isPho && (
                    <>
                      <Link href={`/find/${phoCityParam(p.citySlug, p.stateCode)}`} className="text-brand-ink hover:underline">
                        Pho in {p.city}, {p.stateCode}
                      </Link>
                      <Link href="/find/pho-restaurants" className="text-brand-ink hover:underline">All pho restaurants</Link>
                    </>
                  )}
                  <Link href="/find" className="text-brand-ink hover:underline">Search map</Link>
                  <Link href={`/find/${p.citySlug}-${p.stateCode.toLowerCase()}`} className="text-brand-ink hover:underline">
                  ...
                  <Link href="/blog/is-ramen-healthier-than-pasta" className="text-brand-ink hover:underline">Is ramen healthier than pasta?</Link>
                  <Link href="/blog/what-are-the-healthiest-noodles-you-can-eat" className="text-brand-ink hover:underline">Healthiest noodles</Link>
                </div>
