...
export default async function MisoRamenPage() {
  const cityLinks = getServiceCityLinks('Miso')
  const NATIONWIDE_FILTER = { initialBowls: ["miso"] }
  ...
  return (
    <main className="min-h-screen bg-white">
      <Navbar />
      <PseoListicle
        breadcrumb={[{ label: 'Ramen Near You', href: '/' }, { label: 'Find Ramen', href: '/find' }, { label: "Ramen Miso Near Me" }]}
        title={`Ramen Miso Near Me — ${count} Spot${count === 1 ? '' : 's'}`}
