...
export default function ExperiencesList({ experiences, destinations, categories }: Props) {
  ...
  return (
    <>
      {/* Filters */}
      <div className="bg-[#FAFAF9] border border-black/8 rounded-xl p-3 mb-6">
        <input
          type="text"
          value={query}
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-3">
          {filtered.map((e, i) => (
            <article key={e.slug} className="bg-white border border-black/8 rounded-xl p-4 flex gap-3">
              ...
              <span className="relative w-20 h-20 rounded-lg overflow-hidden shrink-0 bg-[#F5F4F0]">
              ...
              <div className="min-w-0 flex-1">
                <h2 className="font-bold text-sm text-[#1E2026] leading-tight">
                  <Link href={`/experiences/${e.slug}`} className="hover:text-[#96602F] transition-colors">
                    {e.name}
                  </Link>
                </h2>
                ...
                <div className="flex items-center gap-2 flex-wrap mt-1">
                  ...
                  <span className="text-xs font-semibold text-[#1E2026]">{e.rating.toFixed(1)}</span>
                  <span className="text-xs text-[#6B6862]">{e.reviewCount.toLocaleString()} reviews</span>
                ...
                <p className="text-xs text-[#1E2026] mt-1.5 leading-relaxed">{e.tagline}</p>

                <div className="flex flex-wrap items-center gap-2 mt-2">
                  <span className="px-2 py-0.5 rounded-full bg-[#F5F4F0] border border-black/8 text-[10px] font-medium text-[#6B6862]">
                    {e.category}
                  </span>
                  <span className="text-xs font-bold text-[#1E2026]">From {e.priceFrom}</span>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-3 mt-3">
                  <a
                    ...
                    className="inline-flex items-center gap-1.5 px-4 py-2 rounded-lg bg-[#B57F50] hover:bg-[#c8934f] text-white text-xs font-bold transition-colors"
                  >
                    <Ticket className="w-3.5 h-3.5" /> Check Availability
                  </a>
                  <Link
                    href={`/experiences/${e.slug}`}
                    className="inline-flex items-center gap-1 text-xs font-semibold text-[#96602F] hover:underline"
                  >
