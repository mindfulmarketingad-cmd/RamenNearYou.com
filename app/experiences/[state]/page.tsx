...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = `Ramen Experiences in ${g.stateName} | Classes & Food Tours`
  const description = `${g.count} ramen making classes, noodle workshops and Japanese food tours in ${g.stateName}. Compare prices and book.`
  return {
    title,
    description,
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 pt-24 pb-16">
          ...
          <header className="mb-6">
            <h1 className="font-serif text-3xl sm:text-4xl font-bold text-ink leading-tight mb-2">
              Ramen Experiences in {group.stateName}
            </h1>
            <p className="text-ink-soft text-sm">
              {items.length} experience{items.length === 1 ? '' : 's'} you can book, ranked by rating and review volume.
            </p>
          </header>
                  ...
                  <article className="bg-surface border border-line/8 rounded-xl overflow-hidden hover:border-brand/45 transition-colors">
                    <div className="flex items-stretch gap-3 p-3 sm:p-4">
                      ...
                      <div className="min-w-0 flex-1">
                        <h2 className="font-bold text-sm text-ink leading-snug">
                          <Link href={`/experiences/${group.stateSlug}/${e.slug}`} className="hover:text-brand-ink transition-colors">
                            {e.title}
                          </Link>
                        </h2>
                        ...
                        <div className="flex flex-wrap items-center gap-3 mt-2.5">
                          ...
                          <Link
                            href={`/experiences/${group.stateSlug}/${e.slug}`}
