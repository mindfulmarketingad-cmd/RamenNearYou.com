  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-page pt-24 pb-20">
        ...
        {/* Hero */}
        <section className="px-4 sm:px-6 lg:px-8 pb-10 border-b border-line/5">
          <div className="max-w-4xl mx-auto">
            <p className="text-brand-ink text-xs font-medium uppercase tracking-widest mb-3">Restaurant Menu</p>
            <h1 className="font-serif text-4xl sm:text-5xl font-bold text-ink mb-4 leading-tight">
              JINYA Ramen Bar Menu
            </h1>
            <p className="text-ink-soft text-lg leading-relaxed max-w-2xl">
              The full <strong className="text-ink">JINYA Ramen Bar menu</strong> — covering every ramen bowl, small plate, salad, rice bowl, dessert, and cocktail. Items and availability may vary by location.
            </p>
            <div className="mt-6 flex flex-wrap gap-3 text-xs text-ink-soft/60">
              <Link href="/atlanta/ga/jinya-ramen-bar-buckhead" className="hover:text-brand-ink transition-colors underline underline-offset-2">
                JINYA Ramen Bar – Buckhead, Atlanta →
              </Link>
              <Link href="/atlanta/ga/jinya-ramen-bar-poncey-highland" className="hover:text-brand-ink transition-colors underline underline-offset-2">
        ...
        {/* Photo */}
        <section className="px-4 sm:px-6 lg:px-8 py-10 border-b border-line/5">
          <div className="max-w-4xl mx-auto">
            ...
            {menu.map((section) => (
              <div key={section.heading}>
                <div className="flex items-baseline gap-3 mb-5 pb-3 border-b border-line/8">
                  <h2 className="font-serif text-2xl font-bold text-ink">{section.heading}</h2>
                  {section.note && (
                    <span className="text-ink-soft/50 text-xs">{section.note}</span>
                  )}
                </div>
                <div className="grid gap-4">
                  {section.items.map((item) => (
                    <div
                      key={item.name}
                      className="flex flex-col gap-1 p-4 rounded-xl bg-surface border border-line/5"
                    >
                      <div className="flex items-start justify-between gap-4">
                        <p className="font-semibold text-ink text-sm leading-snug">{item.name}</p>
                        {item.note && (
                          <span className="shrink-0 text-[10px] px-2 py-0.5 rounded-full bg-brand/15 text-brand-ink font-medium whitespace-nowrap">
                            {item.note}
                          </span>
                        )}
                      </div>
        ...
        <section className="px-4 sm:px-6 lg:px-8 pt-16">
          <div className="max-w-4xl mx-auto pt-10 border-t border-line/8">
            <p className="text-ink-soft text-sm mb-4">Find JINYA Ramen Bar locations in Atlanta</p>
            <div className="flex flex-wrap gap-3">
              <Link
                href="/atlanta/ga/jinya-ramen-bar-buckhead"
                className="inline-flex items-center gap-2 px-5 py-2.5 rounded-none bg-brand text-white text-sm font-medium hover:bg-brand-hi transition-colors"
              >
                JINYA Buckhead →
              </Link>
              <Link
                href="/atlanta/ga/jinya-ramen-bar-poncey-highland"
                className="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg bg-brand/15 border border-brand/30 text-brand-hi text-sm font-medium hover:bg-brand/25 transition-colors"
              >
