  ...
  if (saved.length === 0) {
    return (
      <div className="bg-[#1E2026] border border-white/10 rounded-xl p-8 text-center">
        <Heart className="w-10 h-10 text-[#77567A]/30 mx-auto mb-3" />
        <p className="text-white text-sm font-medium mb-1">Nothing saved yet</p>
        <p className="text-[#B0B3BB] text-xs mb-4">Hit the Save button on any restaurant listing to build your list.</p>
        <Link
          href="/cities"
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      {saved.map((r) => (
        <Link
          ...
          className="group flex gap-3 bg-[#1E2026] rounded-xl border border-white/5 overflow-hidden hover:border-[#77567A]/40 transition-all duration-200"
        >
          <div className="relative w-20 h-20 bg-[#2F323A] overflow-hidden flex-shrink-0">
            <RestaurantImage src={r.photo} alt={r.name} fill className="object-cover" />
          </div>
          <div className="flex-1 min-w-0 py-2 pr-3">
            <h3 className="font-semibold text-white text-sm leading-snug group-hover:text-[#77567A] transition-colors line-clamp-1">{r.name}</h3>
