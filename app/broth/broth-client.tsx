...
type BrothType = typeof BROTH_TYPES[number]
...
interface Props {
  ...
  topByType: Record<BrothType, BrothCard[]>
  topOverall: BrothCard[]
  perTypeLimit: number
  overallLimit: number
}

export default function BrothClient({ counts, topByType, topOverall, perTypeLimit, overallLimit }: Props) {
  const [selected, setSelected] = useState<BrothType | 'All'>('All')

  const filtered = selected === 'All' ? topOverall : topByType[selected]
  const totalForSelected = selected === 'All' ? counts.All : counts[selected]
  const shownLimit = selected === 'All' ? overallLimit : perTypeLimit
  const isCapped = totalForSelected > filtered.length
          ...
          {filtered.length === 0 ? (
            ...
            <>
              <p className="text-[#6B6862] text-sm mb-5">
                Showing top {filtered.length.toLocaleString()}
                {isCapped ? ` of ${totalForSelected.toLocaleString()}` : ''} restaurant{filtered.length !== 1 ? 's' : ''}
                {selected !== 'All' ? ` — ` : ''}
                {selected !== 'All' && (
                  <Link
                    href={`/${selected.toLowerCase()}-ramen-near-me`}
                    className="text-[#B57F50] hover:underline"
                  >
                    see the full {brothMeta[selected].label.toLowerCase()} list →
                  </Link>
                )}
              </p>
