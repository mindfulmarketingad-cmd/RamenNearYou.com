          ...
          <div className="grid gap-6">
            {blogPosts.map((post) => (
              <Link
                key={post.slug}
                href={`/blog/${post.slug}`}
                className="group block bg-[#2F323A] rounded-xl p-6 sm:p-8 border border-white/5 hover:border-[#B57F50]/40 transition-all"
              >
                ...
                <h2 className="font-serif text-xl sm:text-2xl font-bold text-white mb-2 group-hover:text-[#B57F50] transition-colors">
                  {post.title}
                </h2>
