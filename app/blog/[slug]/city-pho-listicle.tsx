        ...
        <div className="max-w-3xl mx-auto">
          <div>
            ...
            <article className="mt-4">
              <header className="mb-8">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <span className="text-xs text-ink-soft/60">{totalCount} pho spots tracked in {cityName}</span>
                </div>
              ...
              {/* Table of contents */}
              <nav aria-label="Table of contents" className="mb-8 rounded-xl border border-[#16a34a]/25 bg-sunken p-5">
                ...
                <ol className="space-y-1.5">
                  {top5.map((p, i) => (
                    <li key={p.slug} className="flex gap-2.5 text-[15px] leading-snug">
                      ...
                      <a href={`#${p.slug}`} className="text-ink hover:text-[#16a34a] hover:underline">{p.name}</a>
                    </li>
                  ))}
                  <li className="flex gap-2.5 text-[15px] leading-snug">
                    ...
                    <a href="#keep-exploring" className="text-ink hover:text-[#16a34a] hover:underline">Keep Exploring</a>
                  </li>
                </ol>
              </nav>
                    ...
                    <article key={p.slug} id={p.slug} className="scroll-mt-24 bg-surface rounded-2xl border border-line/5 overflow-hidden">
                      <div className="flex flex-col sm:flex-row">
                        ...
                        <div className="flex flex-col flex-1 p-5 sm:p-6 gap-3">
                          <div>
                            <h2 className="font-serif text-xl font-bold text-ink mb-1">{p.name}</h2>
                            <div className="flex flex-wrap items-center gap-2">
                              {p.rating != null && (
                                <>
                                  <StarRating rating={p.rating} />
                                  <span className="text-ink/70 text-xs">{p.rating.toFixed(1)} ({p.reviewCount.toLocaleString()} reviews)</span>
                                </>
                              )}
                            </div>
                          </div>
              ...
              <section id="keep-exploring" className="scroll-mt-24 mt-10 bg-surface rounded-2xl border border-line/5 p-6 sm:p-8">
                ...
                <div className="flex flex-wrap gap-x-5 gap-y-2 text-sm">
                  ...
                  <Link href="/blog/what-is-pho-made-of" className="text-brand-ink hover:underline">What is pho made of?</Link>
                  <Link href="/blog/is-pho-healthy" className="text-brand-ink hover:underline">Is pho healthy?</Link>
                  <Link href="/blog/are-pho-and-ramen-the-same" className="text-brand-ink hover:underline">Are pho and ramen the same?</Link>
                </div>
              </section>
            ...
            <div className="mt-16 pt-8 border-t border-line/8">
              <p className="text-ink-soft text-sm mb-4">Looking for great pho somewhere else?</p>
              <Link
                href="/partners"
