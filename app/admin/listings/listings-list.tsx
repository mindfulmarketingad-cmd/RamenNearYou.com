    ...
    <div className="bg-[#ffffff] rounded-xl border border-black/5 overflow-hidden">
      <div className="p-5">
        ...
        <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-[#6B6862]/70 mb-4">
          ...
          {listing.website && <a href={listing.website} target="_blank" rel="noopener noreferrer" className="text-[#96602F] hover:underline">🌐 Website</a>}
          {listing.owner_name && <span>👤 {listing.owner_name}</span>}
          {listing.owner_email && <a href={`mailto:${listing.owner_email}`} className="text-[#96602F] hover:underline">✉ {listing.owner_email}</a>}
        </div>
