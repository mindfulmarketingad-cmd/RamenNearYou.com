    ...
    <main className="min-h-screen bg-[#2F323A]">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 pt-28 pb-20">
        <div className="mb-8">
          ...
          <h1 className="font-serif text-3xl sm:text-4xl font-bold text-white mb-2">My Restaurants</h1>
          <p className="text-[#B0B3BB] text-sm">Manage content for restaurants you&apos;ve claimed.</p>
        </div>

        {approved.length === 0 && pending.length === 0 && (
          <div className="bg-[#1E2026] border border-white/10 rounded-xl p-8 text-center">
            <p className="text-white mb-2">You haven&apos;t claimed any restaurants yet.</p>
        ...
        {pending.length > 0 && (
          <section className="mb-10">
            <h2 className="text-white text-sm font-semibold uppercase tracking-wide mb-3">Pending Review</h2>
            <div className="space-y-3">
              {pending.map(c => (
                <div key={c.restaurant_slug} className="bg-[#1E2026] border border-amber-500/20 rounded-xl p-5 flex items-start justify-between gap-4">
        ...
        {approved.length > 0 && (
          <section>
            <h2 className="text-white text-sm font-semibold uppercase tracking-wide mb-3">Verified Restaurants</h2>
            <div className="space-y-3">
              {approved.map(c => {
                const r = slugMap.get(c.restaurant_slug)
                return (
                  <div key={c.restaurant_slug} className="bg-[#1E2026] border border-white/10 rounded-xl p-5 flex items-start justify-between gap-4">
                    <div className="min-w-0">
                      <div className="flex items-center gap-2 flex-wrap mb-1">
                        <p className="text-white font-medium">{c.restaurant_name}</p>
                    ...
                    <div className="flex gap-2 shrink-0">
                      {r && (
                        <Link
                          ...
                          className="px-3 py-2 rounded-lg bg-white/5 hover:bg-white/10 text-[#B0B3BB] hover:text-white text-xs font-medium transition-colors"
                        >
                          View
                        </Link>
                      )}
