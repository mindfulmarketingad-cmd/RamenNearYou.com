...
export default function CateringForm() {
  ...
  const inputClass = 'w-full px-4 py-3 bg-surface border border-line/8 rounded-lg text-ink placeholder-ink-faint/60 text-sm outline-none focus:border-brand transition-colors'
  ...
  if (success) {
    return (
      <div className="bg-sunken rounded-xl border border-line/5 p-8 text-center">
        ...
        <h2 className="font-serif text-2xl font-bold text-ink mb-2">Request Received!</h2>
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-sunken rounded-xl border border-line/5 p-8 space-y-5">
      {error && (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label htmlFor="cat-name" className="block text-xs text-ink-soft mb-1.5 uppercase tracking-wide">Your Name <span className="text-red-400">*</span></label>
          <input id="cat-name" value={form.name} onChange={set('name')} placeholder="Full name" required className={inputClass} />
        </div>
        <div>
          <label htmlFor="cat-email" className="block text-xs text-ink-soft mb-1.5 uppercase tracking-wide">Email <span className="text-red-400">*</span></label>
        ...
        <div>
          <label htmlFor="cat-phone" className="block text-xs text-ink-soft mb-1.5 uppercase tracking-wide">Phone</label>
          <input id="cat-phone" type="tel" value={form.phone} onChange={set('phone')} placeholder="(404) 555-0100" className={inputClass} />
        </div>
        <div>
          <label htmlFor="cat-location" className="block text-xs text-ink-soft mb-1.5 uppercase tracking-wide">Event Location</label>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label htmlFor="cat-event-type" className="block text-xs text-ink-soft mb-1.5 uppercase tracking-wide">Event Type <span className="text-red-400">*</span></label>
        ...
        <div>
          <label htmlFor="cat-guest-count" className="block text-xs text-ink-soft mb-1.5 uppercase tracking-wide">Guest Count <span className="text-red-400">*</span></label>
        ...
        <div>
          <label htmlFor="cat-event-date" className="block text-xs text-ink-soft mb-1.5 uppercase tracking-wide">Event Date</label>
          <input id="cat-event-date" type="date" value={form.eventDate} onChange={set('eventDate')} className={`${inputClass} [color-scheme:dark]`} />
        </div>
        <div>
          <label htmlFor="cat-budget" className="block text-xs text-ink-soft mb-1.5 uppercase tracking-wide">Budget (optional)</label>
          <input id="cat-budget" value={form.budget} onChange={set('budget')} placeholder="e.g. $500–$1,000" className={inputClass} />
        </div>
      </div>

      <div>
        <label htmlFor="cat-notes" className="block text-xs text-ink-soft mb-1.5 uppercase tracking-wide">Additional Notes</label>
        <textarea
          id="cat-notes"
          rows={3}
          value={form.notes}
      ...
      <button
        type="submit"
        disabled={loading}
        className="w-full py-3 rounded-none bg-brand hover:bg-brand-hi text-white text-sm font-semibold transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
      >
