...
function StarRating({ rating }: { rating: number | null }) {
  if (!rating) return <span className="text-ink-soft">N/A</span>
  ...
  return (
    <span className="inline-flex items-center gap-0.5 flex-wrap">
      ...
      <span className="ml-1 text-sm font-bold text-ink">{rating.toFixed(1)}</span>
    </span>
  )
}

function BoolCell({ val }: { val: boolean | undefined | null }) {
  if (val == null) return <span className="text-ink-soft text-sm">N/A</span>
  return val ? (
    ...
    <span className="inline-flex items-center gap-1 text-ink-soft text-sm">
...
function buildRows(): Row[] {
  return [
    ...
    },
    {
      ...
      render: r =>
        r.reviewCount
          ? <span className="text-sm text-ink">{r.reviewCount.toLocaleString()}</span>
          : <span className="text-ink-soft text-sm">N/A</span>,
    ...
    },
    {
      ...
      render: r =>
        r.priceRange
          ? <span className="text-sm text-ink font-medium">{r.priceRange}</span>
          : <span className="text-ink-soft text-sm">N/A</span>,
      raw: r => r.priceRange ?? '',
    },
    {
      ...
      render: r => (
        <span
          className={`text-sm font-semibold ${
            r.businessStatus === 'OPERATIONAL' ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-500'
          }`}
        >
    ...
    },
    {
      ...
      render: r =>
        r.subtypes
          ? <span className="text-sm text-ink">{r.subtypes}</span>
          : <span className="text-ink-soft text-sm">N/A</span>,
      raw: r => r.subtypes ?? '',
    },
    /* Location */
    {
      ...
      render: r =>
        r.address
          ? <span className="text-sm text-ink leading-snug">{r.address}</span>
          : <span className="text-ink-soft text-sm">N/A</span>,
      raw: r => r.address ?? '',
    },
    {
      section: 'Location',
      label: 'City',
      render: r => <span className="text-sm text-ink">{r.city}</span>,
      raw: r => r.city ?? '',
    },
    {
      section: 'Location',
      label: 'State',
      render: r => <span className="text-sm text-ink">{r.state}</span>,
      raw: r => r.state ?? '',
    },
    {
      section: 'Location',
      ...
      render: r =>
        r.postalCode
          ? <span className="text-sm text-ink">{r.postalCode}</span>
          : <span className="text-ink-soft text-sm">N/A</span>,
      raw: r => r.postalCode ?? '',
    },
    /* Contact */
    {
      ...
      render: r =>
        r.phone ? (
          <a href={`tel:${r.phone}`} className="text-brand-ink hover:underline text-sm">
            {r.phone}
          </a>
        ) : (
          <span className="text-ink-soft text-sm">N/A</span>
      ...
      render: r =>
        r.website ? (
          <a
            ...
            className="inline-flex items-center gap-1 text-brand-ink hover:underline text-sm"
          >
    ...
    /* Hours */
    {
      section: 'Hours',
      label: "Today's Hours",
      render: r => <span className="text-sm text-ink">{todayHours(r)}</span>,
      raw: r => todayHours(r),
    },
      ...
      day => ({
        ...
        render: (r: Restaurant) => {
          ...
          return h ? (
            ...
            <span className="text-ink-soft text-sm">Closed</span>
          )
        },
    ...
    <main className="min-h-screen bg-sunken">
      ...
      <div className="pt-24 pb-20 px-4 sm:px-6 lg:px-8">
        <div className="max-w-6xl mx-auto">
          ...
          {/* Breadcrumb */}
          <nav className="flex items-center gap-1.5 text-xs text-ink-soft mb-5 flex-wrap">
            <Link href="/" className="hover:text-brand-ink transition-colors">Home</Link>
            <span>/</span>
            <Link href="/compare" className="hover:text-brand-ink transition-colors">Compare</Link>
            <span>/</span>
            <span className="text-ink-soft truncate">{a.name} vs {b.name}</span>
          </nav>

          {/* H1 */}
          <h1 className="font-serif text-3xl sm:text-4xl font-bold text-ink mb-8 leading-tight">
            {a.name} vs {b.name}
          </h1>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-[1fr_72px_1fr] gap-4 sm:gap-3 items-stretch mb-6">
            {/* Card A */}
            <div className="bg-surface rounded-2xl overflow-hidden border border-line/5 shadow-sm flex flex-col">
              ...
              <div className="p-5 flex flex-col gap-1.5">
                <Link
                  ...
                  className="font-serif text-lg font-bold text-ink hover:text-brand-ink transition-colors leading-snug"
                >
                  {a.name}
                </Link>
                ...
                {a.priceRange && (
                  <span className="text-xs text-ink-soft font-medium">{a.priceRange}</span>
                )}
              </div>
            </div>
            ...
            <div className="flex items-center justify-center py-4 sm:py-0">
              <div className="flex items-center justify-center w-14 h-14 rounded-full bg-contrast text-white shadow-lg shrink-0">
            ...
            <div className="bg-surface rounded-2xl overflow-hidden border border-line/5 shadow-sm flex flex-col">
              ...
              <div className="p-5 flex flex-col gap-1.5">
                ...
                {b.priceRange && (
                  <span className="text-xs text-ink-soft font-medium">{b.priceRange}</span>
                )}
              </div>
            </div>
          </div>
          ...
          {(a.description || b.description) && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
              {[a, b].map(r => (
                r.description ? (
                  <div key={r.slug} className="bg-surface rounded-xl border border-line/5 p-5 shadow-sm">
                    <p className="text-xs font-semibold text-brand-ink uppercase tracking-widest mb-2">
                      About {r.name}
                    </p>
          ...
          <div className="flex items-center justify-center gap-3 mb-8 text-xs flex-wrap">
            <Link href={`/compare/${b.slug}-vs-${a.slug}`} className="text-ink-soft hover:text-brand-ink transition-colors">
              ⇄ Swap order
            </Link>
            <span className="text-ink-soft">·</span>
            <Link href={`/compare?a=${a.slug}`} className="text-ink-soft hover:text-brand-ink transition-colors">
              Change Restaurant B
            </Link>
            <span className="text-ink-soft">·</span>
            <Link href={`/compare?a=${b.slug}`} className="text-ink-soft hover:text-brand-ink transition-colors">
              Change Restaurant A
            </Link>
            <span className="text-ink-soft">·</span>
            <Link href="/compare" className="text-ink-soft hover:text-brand-ink transition-colors">
              Start over
            </Link>
          </div>

          {/* Comparison table */}
          <div className="bg-surface rounded-2xl border border-line/5 shadow-sm overflow-hidden">

            {/* Sticky column header */}
            <div className="grid grid-cols-[140px_1fr_1fr] sm:grid-cols-[180px_1fr_1fr] border-b-2 border-line/8 bg-surface sticky top-0 z-10 shadow-sm">
              <div className="px-4 sm:px-5 py-3.5 bg-sunken border-r border-line/5">
                <span className="text-[10px] font-semibold text-ink-soft uppercase tracking-widest">
                  Feature
                </span>
              </div>
              <div className="px-4 sm:px-5 py-3.5 border-r border-line/5">
              ...
              return (
                <div key={section}>
                  ...
                  <div className="grid grid-cols-[140px_1fr_1fr] sm:grid-cols-[180px_1fr_1fr]">
                    <div className="col-span-3 px-4 sm:px-5 py-2 bg-sunken border-y border-line/5">
                  ...
                  {sectionRows.map((row, i) => {
                    ...
                    return (
                      <div
                        key={row.label}
                        className={`grid grid-cols-[140px_1fr_1fr] sm:grid-cols-[180px_1fr_1fr] border-b border-line/5 last:border-0 ${
                          isDiff ? 'bg-amber-50 dark:bg-amber-500/10' : i % 2 === 0 ? 'bg-surface' : 'bg-raised'
                        }`}
                      >
                        {/* Label */}
                        <div
                          className={`px-4 sm:px-5 py-3 border-r border-line/5 flex items-center ${
                            isDiff ? 'border-l-[3px] border-l-amber-400' : ''
                          }`}
                        >
                          <span className="text-xs font-medium text-ink-soft leading-snug">
                            {row.label}
                          </span>
                        </div>

                        {/* Value A */}
                        <div className="px-4 sm:px-5 py-3 border-r border-line/5 flex items-center min-w-0">
          ...
          <div className="mt-3 flex items-center gap-2 text-xs text-ink-soft">
            <span className="w-3 h-3 rounded-sm bg-amber-50 dark:bg-amber-500/10 border-l-2 border-amber-400 inline-block shrink-0" />
          ...
          <div className="mt-12 text-center">
            ...
            <Link
              href="/compare"
              className="inline-flex items-center gap-2 px-6 py-3 rounded-none bg-brand hover:bg-brand-hi text-white text-sm font-semibold transition-colors shadow-md"
            >
