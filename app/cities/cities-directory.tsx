    ...
    <>
      <div className="relative mb-8">
        ...
        <input
          ...
          className="w-full pl-10 pr-10 py-3 text-sm bg-sunken border border-line/12 rounded-xl outline-none text-ink placeholder-ink-faint focus:border-brand transition-colors"
        />
        {q && (
          <button
            onClick={() => setQ('')}
      ...
      {matches ? (
        <div>
          {matches.length === 0 ? (
            <p className="text-sm text-ink-soft">No cities match &ldquo;{q}&rdquo;.</p>
          ) : (
            <ul className="columns-2 sm:columns-3 md:columns-4 gap-x-6 space-y-1">
              {matches.map((m) => (
                <li key={m.href} className="break-inside-avoid">
                  <Link href={m.href} className="block text-sm text-ink hover:text-brand-ink hover:underline py-1 transition-colors">
                    {m.label}
                  </Link>
                </li>
              ))}
          ...
          {matches.length >= CITY_RESULT_CAP && (
            <p className="text-xs text-ink-soft mt-3">Showing the first {CITY_RESULT_CAP} cities — keep typing to narrow it down.</p>
          )}
        </div>
      ) : (
        <>
          ...
          <div className="flex flex-wrap gap-2 mb-10">
            {statesWithCities.map((s) => (
              <a
                ...
                className="px-3 py-1.5 rounded-full bg-sunken border border-line/8 text-xs font-medium text-ink hover:border-brand/40 hover:text-brand-ink transition-colors"
              >
                {s.state}
              </a>
            ))}
            ...
            {statesWithCities.map((s) => (
              <div key={s.stateSlug} id={`state-${s.stateSlug}`} className="scroll-mt-24">
                <h2 className="flex items-baseline gap-2 mb-3">
                  <Link href={`/${s.stateSlug}`} className="font-serif text-xl font-bold text-ink hover:text-brand-ink transition-colors">
                    {s.state}
                  </Link>
                ...
                <ul className="columns-2 sm:columns-3 md:columns-4 gap-x-6 space-y-1">
                  {s.cities.map((c) => (
                    <li key={c.citySlug} className="break-inside-avoid">
                      ...
                      >
                        {c.city} <span className="text-ink-faint text-xs">({c.count})</span>
                      </Link>
                    </li>
                  ))}
                </ul>
